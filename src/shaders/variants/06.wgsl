...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.222, 0.091, 0.314); let b = vec3<f32>(0.570, 0.316, 0.729); let c = vec3<f32>(0.877, 0.475, 0.300); let d = vec3<f32>(0.157, 0.688, 0.584); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.420) * 0.18;
