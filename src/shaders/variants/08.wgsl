...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.236, 0.098, 0.325); let b = vec3<f32>(0.553, 0.341, 0.739); let c = vec3<f32>(0.882, 0.484, 0.317); let d = vec3<f32>(0.162, 0.691, 0.595); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.560) * 0.18;
