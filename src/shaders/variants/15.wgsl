...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.285, 0.123, 0.364); let b = vec3<f32>(0.494, 0.429, 0.773); let c = vec3<f32>(0.902, 0.513, 0.376); let d = vec3<f32>(0.182, 0.701, 0.634); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 1.050) * 0.18;
