...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.250, 0.105, 0.336); let b = vec3<f32>(0.536, 0.366, 0.749); let c = vec3<f32>(0.888, 0.492, 0.334); let d = vec3<f32>(0.168, 0.694, 0.606); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.700) * 0.18;
