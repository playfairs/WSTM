...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.264, 0.112, 0.347); let b = vec3<f32>(0.519, 0.391, 0.759); let c = vec3<f32>(0.894, 0.500, 0.351); let d = vec3<f32>(0.174, 0.697, 0.617); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.840) * 0.18;
