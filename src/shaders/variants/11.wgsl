...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.257, 0.109, 0.342); let b = vec3<f32>(0.528, 0.379, 0.754); let c = vec3<f32>(0.891, 0.496, 0.342); let d = vec3<f32>(0.171, 0.695, 0.612); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.770) * 0.18;
