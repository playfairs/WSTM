...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.201, 0.081, 0.297); let b = vec3<f32>(0.595, 0.278, 0.715); let c = vec3<f32>(0.868, 0.463, 0.275); let d = vec3<f32>(0.148, 0.684, 0.567); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.210) * 0.18;
