...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.313, 0.137, 0.386); let b = vec3<f32>(0.460, 0.479, 0.793); let c = vec3<f32>(0.913, 0.530, 0.410); let d = vec3<f32>(0.193, 0.707, 0.656); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 1.330) * 0.18;
