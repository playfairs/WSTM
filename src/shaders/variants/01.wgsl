...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.05,0.3,0.55); let b = vec3<f32>(0.92,0.72,0.2); let c = vec3<f32>(0.9,0.4,0.65); let d = vec3<f32>(0.2,0.7,0.25); return a + b * cos(6.28318 * (c*t + d)); }
...
@fragment fn sim_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let t = uniforms.time * (0.6 + abs(uniforms.chaos_seed)) + uniforms.frame * 0.0014;
    let energy = clamp(uniforms.mid * 3.0 + uniforms.treble * 2.5 + uniforms.rms * 1.4, 0.0, 1.0);
    let r = abs(length(p) - 0.24);
    let a = atan2(p.y, p.x);
    let rings = smoothstep(0.08, 0.0, r) * (0.4 + energy * 0.9);
    let dust = fbm(p * 5.0 + vec2<f32>(t, -t * 0.4)) * 0.7;
    let color = palette(uniforms.hue_shift + dust * 0.3 + sin(a * 5.0 + t) * 0.1) * (0.3 + rings) + palette(0.6 + uniforms.hue_shift) * dust * 0.25;
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.75 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.3) * 0.2;
