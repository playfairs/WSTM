...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.243, 0.102, 0.330); let b = vec3<f32>(0.544, 0.353, 0.744); let c = vec3<f32>(0.885, 0.488, 0.326); let d = vec3<f32>(0.165, 0.693, 0.600); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.630) * 0.18;
