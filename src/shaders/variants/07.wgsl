...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.229, 0.095, 0.319); let b = vec3<f32>(0.561, 0.328, 0.734); let c = vec3<f32>(0.880, 0.479, 0.309); let d = vec3<f32>(0.160, 0.690, 0.589); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.490) * 0.18;
