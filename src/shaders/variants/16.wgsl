...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.292, 0.126, 0.370); let b = vec3<f32>(0.486, 0.442, 0.778); let c = vec3<f32>(0.905, 0.517, 0.384); let d = vec3<f32>(0.185, 0.702, 0.640); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 1.120) * 0.18;
