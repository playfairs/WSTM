...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.306, 0.133, 0.381); let b = vec3<f32>(0.469, 0.467, 0.788); let c = vec3<f32>(0.910, 0.526, 0.401); let d = vec3<f32>(0.190, 0.705, 0.651); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 1.260) * 0.18;
