...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.271, 0.116, 0.353); let b = vec3<f32>(0.511, 0.404, 0.764); let c = vec3<f32>(0.896, 0.505, 0.359); let d = vec3<f32>(0.176, 0.698, 0.623); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.910) * 0.18;
