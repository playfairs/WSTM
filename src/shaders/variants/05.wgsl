...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.215, 0.088, 0.308); let b = vec3<f32>(0.578, 0.303, 0.724); let c = vec3<f32>(0.874, 0.471, 0.292); let d = vec3<f32>(0.154, 0.687, 0.578); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.350) * 0.18;
