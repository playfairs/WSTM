...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.208, 0.084, 0.302); let b = vec3<f32>(0.586, 0.290, 0.720); let c = vec3<f32>(0.871, 0.467, 0.284); let d = vec3<f32>(0.151, 0.686, 0.572); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 0.280) * 0.18;
