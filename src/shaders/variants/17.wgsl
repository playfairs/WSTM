...
fn palette(t: f32) -> vec3<f32> { let a = vec3<f32>(0.299, 0.130, 0.375); let b = vec3<f32>(0.477, 0.454, 0.783); let c = vec3<f32>(0.908, 0.521, 0.393); let d = vec3<f32>(0.188, 0.704, 0.645); return a + b * cos(6.28318 * (c * t + d)); }
...
@fragment fn display_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let color = previous * 0.76 + palette(uniforms.hue_shift + uniforms.pattern_seed * 0.26 + 1.190) * 0.18;
